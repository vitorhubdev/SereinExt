...
pub(super) fn open(
	...
	let name = random
		...
		.collect::<String>();
	let directory = std::env::temp_dir().join(format!("nivra-video-{name}"));
	fs::DirBuilder::new()
		.mode(0o700)
		.create(&directory)
		.map_err(|_| FAILED)?;
	...
	fs::remove_dir_all(&temporary.0).map_err(
		|_| "Temporary video cleanup failed; remove nivra-video files from the system temporary folder",
	)?;
	result
}
...
fn convert(
	...
	let mut child = Command::new("/bin/sh")
		.args(["-c", "ulimit -f 131072 || exit 1; exec \"$@\"", "nivra-video", executable])
