...
impl CaptchaView {
	...
	fn build(
		...
		let builder = WebViewBuilder::new()
			...
			.with_custom_protocol("nivra-captcha".into(), move |_, request| {
				let valid = request.method() == "GET" && request.uri() == "nivra-captcha://verification.invalid/";
			...
			.with_url("nivra-captcha://verification.invalid/")
...
pub(super) fn page(
	...
	let script =
		include_str!("captcha.js").replace("__NIVRA_CAPTCHA_CONFIG__", &config.to_string());
...
fn own_origin(value: &str) -> bool {
	url::Url::parse(value).is_ok_and(|url| {
		((url.scheme() == "https"
			&& url.host_str() == Some("nivra-captcha.verification.invalid")
			&& url.port_or_known_default() == Some(443))
			|| (cfg!(target_os = "macos")
				&& url.scheme() == "nivra-captcha"
...
mod tests {
	...
	fn ipc_is_restricted_to_the_local_verification_page() {
		...
		for url in [
			"https://discord.com/",
			"https://nivra-captcha.verification.invalid.evil.test/",
			"http://nivra-captcha.verification.invalid/",
			"https://user@nivra-captcha.verification.invalid/",
			"https://nivra-captcha.verification.invalid:444/",
			"https://nivra-captcha.verification.invalid/other",
		] {
	...
	fn native_local_page_and_ipc() {
		...
		impl ApplicationHandler for App {
			fn resumed(&mut self, event_loop: &ActiveEventLoop) {
					...
					event_loop
						.create_window(
							Window::default_attributes()
								.with_title("Nivra — offline verification smoke")
				...
				let script = r#"(() => { const config = __NIVRA_CAPTCHA_CONFIG__; document.addEventListener('DOMContentLoaded', () => { if(window === window.top && location.origin === 'https://nivra-captcha.verification.invalid' && document.getElementById('captcha')) { document.getElementById('status').textContent = 'Offline verification surface loaded.'; window.ipc.postMessage(config.capability + 'verified:synthetic-passcode'); } }); })();"#;
